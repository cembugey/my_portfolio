const styles = {
  ...
  sectionSubTextLight:
    'sm:text-[18px] text-[16px] text-taupe uppercase tracking-wider font-semibold font-poppins',
     // Hero section styles
  // heroSection: 'relative w-full h-screen mx-auto flex flex-col sm:flex-row',
  // worldMapBg:
  //   'absolute top-0 left-0 w-1/2 h-1/2 bg-no-repeat bg-contain bg-top-left',
  // bgProgrammingBg:
  //   'absolute bottom-0 right-0 w-1/2 h-1/2 bg-no-repeat bg-contain bg-bottom-right',

  // // Adjust the sizes as needed
  // worldMapBgLarge:
  //   'absolute top-0 left-0 w-full h-full bg-no-repeat bg-cover bg-top-left',
  // bgProgrammingBgLarge:
  //   'absolute bottom-0 right-0 w-full h-full bg-no-repeat bg-cover bg-bottom-right',
};
